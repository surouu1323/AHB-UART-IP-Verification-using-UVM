class uart_function_otf_test extends uart_base_test;
  `uvm_component_utils(uart_function_otf_test)

  int send_random_data_time = 1; // Number of data packets to send per test
  int test_case_cnt = 0;         // set number of test
  int test_cnt_set  = 10;         // set number of test
  logic [7:0] rand_val; 
  ...
  function new(string name="uart_function_otf_test", uvm_component parent);
  ...
  virtual task run_phase(uvm_phase phase); 
	...
	fork
		...
		begin
			#1ms; 
			ahb_vif.HRESETn = 'b0; #10ns;  ahb_vif.HRESETn = 'b1;
		end
	join_any
	disable fork;
   ...
   endtask
	...
	reg_cfg();
		...
		repeat(send_random_data_time) begin
			i++;
			`uvm_info("UART_DATA", $sformatf("Iteration [%0d/%0d] -> Writing Data" ,   i+1, send_random_data_time), UVM_LOW)
