class uart_half_tx_test extends uart_base_test;
  `uvm_component_utils(uart_half_tx_test)

  int send_random_data_time = 1; // Number of data packets to send per test
  int test_case_cnt = 0;         // set number of test
  logic [7:0] rand_val; 
  ...
  function new(string name="uart_half_tx_test", uvm_component parent);
    ...
    done_tx_transfer_ev = uvm_event_pool::get_global("DONE_RX_TRANSFER");
    done_rx_transfer_ev = uvm_event_pool::get_global("DONE_TX_TRANSFER");
    endfunction
  ...
  virtual task run_phase(uvm_phase phase); 
	...
	rand_val =32'h20;
//	uart_cfg.baud_rate   = baud_list[6];
//	uart_cfg.data_width  = width_list[3];
//	uart_cfg.stop_bit    = stop_list[0];
//	uart_cfg.parity_mode = parity_list[2];

   
	uart_cfg.baud_rate    = uart_configuration::B_9600;
	uart_cfg.data_width   = uart_configuration::D_6b;
	uart_cfg.stop_bit     = uart_configuration::STOP_2BIT;
	uart_cfg.sampling_mode     = uart_configuration::M_16x;

	for(int i =0; i<8; i++)begin
	
	rand_val =(32'hFF & (8'h1 << i));
	uart_cfg.parity_mode  = uart_configuration::NONE;
